...
class Client extends events.EventEmitter {
   ...
   strip_color(text) {
      text = String(text)
      return text.replace(
         /\x03\d{0,2},?\d{1,2}|[\x02\x06\x07\x0f\x16\x17\x1b\x1d\x1f]/g,
         '',
      )
   }
...
class Channel extends events.EventEmitter {
   ...
   self_command(command, message, options) {
   ...
   send_reply(user, message, options) {
      if (user === this) {
