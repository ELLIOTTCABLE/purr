...
class Bot extends events.EventEmitter {
   ...
   init() {
      ...
      for (var i = 0, len = this.__profile.length; i < len; i++) {
         ...
         this.log.call(this.LOG_CONNECT, client.name, 'Connecting...')
         client.bot = this
         client.connect()
      }
   }
   ...
   LOG_JOIN = 2 // Log channel joins/parts
   ...
   parse_intent(str) {
      var parsed = str.match(
         /^(.*?)\s*@\s*([A-Z\x5B-\x60\x7B-\x7D][-0-9A-Z\x5B-\x60\x7B-\x7D]{0,16})$/i,
      )
   ...
   listeners = {
      disconnect: function (client, why) {
         ...
         setTimeout(function () {
            Bot.prototype.log.call(bot, bot.LOG_CONNECT, client.name, 'Connecting...')
      ...
      pm: function (user, text) {
         ...
         var command_matches
         if ((command_matches = text.match(this.__commands_regex))) {
