      ...
      message: function (message) {
         ...
         var context = {
            ...
            send_to_intents: function (text, options) {
               if (Array.isArray(this.intents) && this.intents.length > 0) {
         ...
         switch (message.channel.type) {
            case 'dm':
               ...
               context.has_access = (() => {
                  for (var guild of this.__client.guilds.cache.values()) {
                     var member = guild.members.cache.get(message.author.id)

                     if (member) {
                        for (var role of member._roles) {
                           if (this.__profile.access_roles.has(role)) {
                              return true
                           }
                        }
                     }
            ...
            case 'text':
               ...
               context.priv = false
               context.has_access = message.member._roles.find((id) =>
                  this.__profile.access_roles.has(id),
               )
